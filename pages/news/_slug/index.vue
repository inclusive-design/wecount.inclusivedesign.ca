    ...
    <div style="width: 80%;">
      ...
      <div>
        ...
        <span v-for="(t, index) in Tags" :key="index">
          <nuxt-link v-if="(index+1)===Tags.length" :to="{ path: '/tag', query: { s: t.tag }}">{{ t.tag }} </nuxt-link>
          <nuxt-link v-else :to="{ path: '/tag', query: { s: t.tag }}">{{ t.tag }},  </nuxt-link>
        </span>
      </div>
      <br>
      <br>
...
export default {
	...
	methods: {
		...
		pageGen (pageNum) {
			return this.linksNames[pageNum - 1]
		}
	}
}
</script>
